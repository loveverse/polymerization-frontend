...
const MenuManage: React.FC = () => {
  ...
  const columns: TableColumnsType<MenuListRes> = [
    ...
    {
      title: "菜单类型", dataIndex: "menuType", render: (_value, record) => {
        return <Tag
          color={record.menuType === "2" ? "success" : "processing"}>{dict.getDictItemMap("menu_type", record.menuType)}</Tag>
      }
    },
  ...
  return (
    <div className={styles.root}>
      <Space className="operation-header">
        <Select value={moduleId} options={moduleList}
                fieldNames={{value: "id", label: "moduleName"}}
                style={{width: 150}} onChange={(value) => {
          setModuleId(value);
