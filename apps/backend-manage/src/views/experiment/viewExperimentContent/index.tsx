...
import { reqViewExperimentContent } from "@/api/aiModel";
import { useLocation } from "react-router-dom";
import { Space, Table, TableColumnsType, Tag, Tooltip, message } from "antd";
import { ComTitle } from "@/components";
import { useAppContext } from "@/context";
import { ViewExperimentRes } from "@/api/aiModel/types";

const ViewExperimentContent = () => {
    ...
    <div className={styles["root"]}>
      ...
      <div className="experiment-content">
        ...
        <div className="report">
          <ComTitle title="实验报告" />
          <ul>
            {experimentInfo.questionList
              ? experimentInfo.questionList.map((item) => {
                  return (
                    <li key={item.id}>
                      <div
                        ...
                        {item.questionType === "CHOICE" ? (
                          <Space size={20}>
                            {item.expands.map((n: any) => {
                              return (
                                <Space key={n.id} size={5}>
                                  <input type="checkbox" name={n.id} checked={n.answer} readOnly />
                                  <label htmlFor={n.id}>{n.content}</label>
                                </Space>
                              );
                            })}
                          </Space>
...
export default ViewExperimentContent;
