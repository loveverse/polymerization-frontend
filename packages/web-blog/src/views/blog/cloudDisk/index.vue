<template>
  <div class="wrapper">
    ...
    <el-table :data="state.allFileList" style="width: 100%; margin-top: 20px" v-loading="loading">
      <el-table-column
        type="index"
        :label="$t('number')"
        width="80"
      ...
      <el-table-column :label="$t('size')" prop="fileSize" align="center"></el-table-column>
      <el-table-column :label="$t('operation')" align="center">
        <template #default="{ row }">
          <div class="operation">
            <span
              ><a :href="row.fileUrl" class="save">{{ $t("download") }}</a></span
            >
            <span v-if="user" @click="handleBeforeDel(row.id)">{{ $t("delete") }}</span>
          </div>
        </template>
      </el-table-column>
    </el-table>
    <el-drawer
      ...
      <el-upload
        ...
        <template #file="{ file }">
          <div class="img_info">
            <el-image :src="file.url" class="file_img" :preview-src-list="[file.url]">
              <template #error>
                <el-icon>
                  <DocumentRemove />
                </el-icon>
          ...
          <el-progress v-show="file.showProgress" :percentage="file.percentage" />
        </template>
      </el-upload>
      <el-progress v-show="state.showProgress" :percentage="state.loadProgress" />
    </el-drawer>
    <ComPreview
...
<script lang="ts" setup name="cloudDisk">
import { useThrottle } from "@/hooks/useThrottle.ts"

import { UploadProps, FormInstance } from "element-plus"
import { DocumentRemove, Upload, UploadFilled } from "@element-plus/icons-vue"
import { reqFileList, reqSaveFile, reqDelFile } from "@/api/blog/cloudDisk"
import { reqUpload, reqPasteUpload } from "@/api/base"
import { FILE_TYPE } from "@/utils/constant.ts"
import { t } from "@/lang"
...
const handleCommonDrawer = (flag: boolean) => {
  ...
  state.showUpload
    ? document.addEventListener("paste", useThrottle(handlePaste, 500))
...
const uploadImage = async (dataUrl: any) => {
  ...
  params.append("file", dataUrl.replace(/^data:image\/(png|jpg|jpeg);base64,/, ""))
  ...
  if (result.code === 200) {
    ...
    let data = import.meta.env.DEV ? { ...result.data, url: dataUrl } : result.data
