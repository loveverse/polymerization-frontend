<template>
  <div ref="contentRef" class="wrapper">
    <ul v-loading="loading" class="out" :class="state.flag && 'out_pad'">
      <li v-for="item in state.findData" :key="item.id" class="card">
        <div class="artcle">
          <el-input
          ...
          <template v-else>
            ...
            <p v-else class="title" :data-id="item.id" @click="edit($event, item.content)">
              {{ item.content }}
            </p>
          </template>
          <div class="tip">
            ...
            <p v-if="item.author" class="author">{{ t("comeFrom1") }}{{ item.author }}”</p>
          </div>
        </div>
        <el-button v-show="state.flag" type="danger" size="small" class="del" @click="del(item.id)">
          {{ t("delete") }}
        </el-button>
      </li>
    </ul>
    <div class="site_info">
      <el-tag type="success">
        {{ t("total") }} &emsp;{{ state.findData.length }}&emsp;{{ t("article") }}
      </el-tag>
      <el-result
        class="status"
    ...
    <div class="ipt_text">
      <el-input
        ...
        autosize
        placeholder="请输入内容" />
      <el-input v-model.trim="state.author" clearable placeholder="请输入作者" maxlength="30" />
...
<script setup lang="ts" name="chatRoom">
  ...
  const scrollBottom = () => {
    ...
    nextTick(() => {
      // 滚动到底部
      const h: any = document.getElementById("main")
